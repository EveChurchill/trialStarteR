#' Allocates single ID occurrence dataframe vector to dataframe with multiple
#'   events
#'
#' @description
...
#'
#' @param input_dataframe (data frame) This the data frame that we want to add
#'   the variables to.
#'
#' @param id_cols (vector) the names of the id columns as a character vector.
#'   Default is `c("screening", "event_name", "event_id")`.
#'
#' @param vector_to_allocate (vector) This is the variable's data vector we want
#'   to add to the data frame.
#'
#' @param correspond.indivID.vector (vector) This is the corresponding ID list
#'   for vector_to_allocate
#'
#' @returns (vector) returns a vector with the variable's elements in the
...
#' @export
insert_vectors_with_single_screeningID <- function(input_dataframe,
                                                   id_cols,
                                                   vector_to_allocate,
  ...
  for (i in input_dataframe[, c(id_cols[1])]) {
    if (i %in% correspond.indivID.vector) {
      allocated <- append(allocated, vector_to_allocate[which(correspond.indivID.vector ==
                                                                i)])
    } else {
