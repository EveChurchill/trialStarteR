...
#'
#' @param variable.details.df (dataframe) the dataframe within the inputs folder which specifies which variables to be seen in the screening table - typically called variable.details.df
#'
...
construct_screening_table<-function(trial.data, variable.details.df=variable.details.df){
  all.variables.ordered<-variable.details.df$VariableProspectName[variable.details.df$screening_yn=='y']
  ...
  characteristic_data<-df[df$event_name=='Screening', c('screening', categorical, continuous)]
